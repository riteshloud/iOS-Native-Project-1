...
public class CITreeView: UITableView {
    ...
    fileprivate func deleteRows() {
        if treeViewController.indexPathsArray.count > 0 {
            self.beginUpdates()
            self.deleteRows(at: treeViewController.indexPathsArray, with: .none)
            self.endUpdates()
        }
    }
    ...
    fileprivate func insertRows() {
        if treeViewController.indexPathsArray.count > 0 {
            self.beginUpdates()
            self.insertRows(at: treeViewController.indexPathsArray, with: .none)
            self.endUpdates()
        }
    }
...
extension CITreeView : UITableViewDataSource {
    public func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        debugPrint("treeViewController.treeViewNodes.count: \(treeViewController.treeViewNodes.count)")
        self.treeViewDelegate?.treeViewNoOfNodes(intNoOfNodes: treeViewController.treeViewNodes.count)
